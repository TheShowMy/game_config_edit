...
结构化类型使用类似 TypeScript 的表达式：

- 数组：`string[]`、`number[][]`、`{id:number}[]`
- 对象：`{mid:number;name:string}`
- 可选字段：`{id:number;name?:string}`
- 异构数组：`(number|string)[]`
- 空数组：`unknown[]`

程序会合并整列中的合法结构值。例如某些对象有 `name` 字段、另一些没有时，类型会显示为 `{id:number;name?:string}`。只要一列出现合法 JSON 对象，该列其他非空值也必须是 JSON 对象；数组列遵循相同规则，不匹配的单元格会标红。
...
完整产品行为和验收标准见 [`docs/requirements.md`](docs/requirements.md)，性能基线见 [`docs/performance.md`](docs/performance.md)。
